...
const getCustomers = (customers, countries) => {
  return new Promise((resolve, reject) => {
    let verifiedCustomers = customers.filter(customer => customer.verified);
    let joinedArrays = [];

    verifiedCustomers.forEach(verifiedCustomer => {
      const country = countries.find(country => country.id === verifiedCustomer.id);

      if(!country) {
        reject(`We don't have information about country for this customer: ${customer.name}`);
      }

      joinedArrays.push({ ...verifiedCustomer, ...country });
      resolve(joinedArrays);
    });
  });
};
